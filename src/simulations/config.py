...
Star particle density must be an integer. Got: %s""" % (type(value)))
	...
	def setup_nthreads(self):
		...
		The number of threads to use on setup (i.e. finding analog star
		particles).
		"""
		return self._setup_nthreads

	@setup_nthreads.setter
	def setup_nthreads(self, value):
		if isinstance(value, numbers.Number):
			if value % 1 == 0:
				self._setup_nthreads = int(value)
			else:
				raise ValueError("Setup nthreads must be an integer. Got: %g" % (
					value))
		else:
			raise TypeError("Setup nthreads must be an integer. Got: %s" % (
				type(value)))
	...
	def nthreads(self):
		...
		The number of threads to use in integrating the GCE model after setup.
		"""
	...
	def nthreads(self, value):
		if isinstance(value, numbers.Number):
			if value % 1 == 0:
				...
				raise ValueError("Nthreads must be an integer. Got: %g" % (
					value))
		else:
			raise TypeError("Nthreads must be an integer. Got: %s" % (
				type(value)))
